...
// GAME: Audio files
// Function to initialize audio files, allowing me to specify a global volume for all of them
...
const volumeSetting = 0.25;

const paddleAudio = createAudio("./sound/paddle.wav", volumeSetting);
const lostBallAudio = createAudio("./sound/lostball.wav", volumeSetting);
const wallAudio = createAudio("./sound/wall.wav", volumeSetting);
const brickAudio = createAudio("./sound/brick.wav", volumeSetting);
const ironBallAudio = createAudio("./sound/ironball.wav", volumeSetting);
const youloseAudio = createAudio("./sound/_youlose.wav", volumeSetting);
const youwinAudio = createAudio("./sound/_youwin.wav", volumeSetting);
...
// GAME PROPS: Bricks
const colors = ["255, 73, 158", "210, 100, 182", "164, 128, 207", "119, 155, 231", "73, 182, 255", "156, 255, 250", "172, 243, 157", "254, 254, 120"];
const brickInfo = {
  w: 70,
  h: 20,
  padding: 10,
...
for (let i = 0; i < brickRowCount; i++) {
  ...
  for (let j = 0; j < brickColumnCount; j++) {
    ...
    const fillColor = `rgba(${colors[j]}, 1)`; // this will make every row a different color from the colors array above
    const borderColor = `rgba(${colors[j]}, 0.5)`; // this will make the border different on each row, following the same system
    bricks[i][j] = { x, y, fillColor, borderColor, ...brickInfo };
  }
}
...
function drawBricks() {
  bricks.forEach((column) => {
    column.forEach((brick) => {
      ...
      ctx.fill();
      ctx.strokeStyle = brick.visible ? brick.borderColor : "transparent";
      ctx.lineWidth = 5;
      ctx.stroke();
      ctx.closePath();
    });
